...
def theta_method_pde_solver(
    ...
    B = -1 / dtime - (2 * a_coeff(state) / dstate**2 + 3 * b_coeff(state) / (2 * dstate)) * theta + c_coeff(state)
    ...
    b = -1 / dtime + (2 * a_coeff(state) / dstate**2 + 3 * b_coeff(state) / (2 * dstate)) * (1 - theta)
    c = -(a_coeff(state[:-1]) / dstate**2 + 2 * b_coeff(state[:-1]) / dstate) * (1 - theta)
    d = b_coeff(state[:-2]) / (2 * dstate) * (1 - theta)

    coeff_matrix_current = diags_array((A, B, C, D), offsets=(-1, 0, 1, 2), format="csc")
